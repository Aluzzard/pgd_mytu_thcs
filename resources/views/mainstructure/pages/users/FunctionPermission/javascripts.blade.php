...
$(document).ready(function() {
    ...
    func_formReset = function(){
        $('input[type="checkbox"]').attr('checked', false); $('input[type="checkbox"]').parent().removeClass('checked');
    }
    /* ------------------------------------------ Handle ------------------------------------------*/
                        ...
                        for (var i = 0; i < data.list.length; i++) {
                            if(data.list[i].function_permissions != null) {
                                for (var j = 0; j < data.list[i].function_permissions.split(',').length; j++) {
                                    $('#form-action input[value='+data.list[i].action+'][data-id-module='+data.list[i].function_permissions.split(',')[j]+']').parent().addClass('checked');
                                    $('#form-action input[value='+data.list[i].action+'][data-id-module='+data.list[i].function_permissions.split(',')[j]+']').attr('checked', true);
                                }
                            } else {
