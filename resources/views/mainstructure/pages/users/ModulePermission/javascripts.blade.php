...
$(document).ready(function() {
    func_buttonClick = function(p_type, p_id){
        if ( p_type == 'edit') {
            $('#actionModal .modal-title').text("Phân quyền module người dùng");
    ...
    func_formReset = function(){
        $('#form-action input[type="checkbox"]').attr('checked', false); $('#form-action input[type="checkbox"]').parent().removeClass('checked');
    }
    /* ------------------------------------------ Handle ------------------------------------------*/
    $("#form-action").on("submit", function( event ) {
        ...
        $data = {
            ...
            array_module_permissions: func_checkRole(),
            user_id: $('#form-action #id').val()
        };
        $url = '{!! route('user.action.module.permission') !!}';
                        ...
                        for (var i = 0; i < data.list.length; i++) {
                            if(data.list[i].module_permissions != null) {
                                for (var j = 0; j < data.list[i].module_permissions.split(',').length; j++) {
                                    $('#form-action input[value="'+data.list[i].action+'"][data-id-module="'+data.list[i].module_permissions.split(',')[j]+'"]').parent().addClass('checked');
                                    $('#form-action input[value="'+data.list[i].action+'"][data-id-module="'+data.list[i].module_permissions.split(',')[j]+'"]').attr('checked', true);
                                }
                            } 
                            else {
                                $('#form-action input[value='+data.list[i].action+']').each( function () {
                                    $('#form-action input[value="'+data.list[i].action+'"]').parent().removeClass('checked');
                                    $('#form-action input[value="'+data.list[i].action+'"]').attr('checked', false);
                                });
                            }
                        }
                    } else {
