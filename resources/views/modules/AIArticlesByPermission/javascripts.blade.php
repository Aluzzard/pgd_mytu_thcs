...
$(document).ready(function() {
    ...
    func_chooseArticlesByCategory = function(id){
        $.ajax({
            type: 'POST',
            url: '{!! route('AIArticlesByPermission.post.listArticles') !!}',
            data: {
                category_id: id,
            },
            dataType:'JSON',
            success: function(data) {
                ...
                if(data.error == false){
                    ...
                    data.check_add == false ? $('#btn_add').hide() : $('#btn_add').show();
                }
            },
            error: function(data) { 
                func_LoadTree();
    ...
    func_LoadTree = function() {     
        $.ajax({
            ...
            data: {
                action_type: 'treeviewbypermission'
            },
            dataType:'JSON',
            success: function(data) {
    ...
    $("#form-action").on("submit", function( event ) {
        ...
        if ( func_checkValidate() == true ) {
            $.ajax({
                type: 'POST',
                url: '{!! route('AIArticlesByPermission.post.ajax') !!}',
