    	...
    	try {
    		...
    		if ( $action_type == 'view') {
	    		...
	    		if($request->hasFile('avatar')){
	                ...
	                $file->move(public_path().'/upload/avatar_account',$file_name);
	                $user->avatar_path = '/upload/avatar_account/'.$file_name;
		    	}
		    	$user->name = $request->input('name');
		    	$user->email = $request->input('email');
		    	$user->numberphone = $request->input('numberphone');
				$user->update();
				return response()->json(['error'=>false, 'message'=>'Thay đổi thông tin thành công!', 'reload_html' => $user]);
    		}
    	} catch (Exception $e) {
