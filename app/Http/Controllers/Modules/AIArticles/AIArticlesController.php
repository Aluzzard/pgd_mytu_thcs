<?php
namespace App\Http\Controllers\Modules\AIArticles;
...
class AIArticlesController extends Controller {
    ...
    public function index() {
        if(CheckRoleByUser::checkRoleModuleByUser('view') == false || CheckRoleByUser::checkRoleFunctionByUser('view',1) == false){
    	    ...
    	    return view('modules.AIArticles.index', $data);
        }
    }
    ...
    public function listArticles(Request $request) {
        try{
            ...
            $html = $this->dataTable($articles);
            return response()->json(['error'=>false, 'table_html' => $html]);
        } catch (Exception $e) {
    ...
    public function dataTable($articles){
        $html = '';
        if($articles){
