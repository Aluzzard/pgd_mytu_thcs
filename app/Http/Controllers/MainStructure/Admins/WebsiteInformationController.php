...
class WebsiteInformationController extends Controller {
	...
	public function index(){
		...
		if (Auth::guard('user')->check() == true){
			if( CheckRoleByUser::checkRoleFunctionByUser('view', 5) == false ){
	    		abort('404');
	    	} else {
	...
	public function ajax(Request $request){
		...
		if(Auth::guard('user')->check()){
			if ($action_type == 'view_website_information' ) {
				if( CheckRoleByUser::checkRoleFunctionByUser('view', 5) == true ){
		...
		try {
            ...
            if($validator->fails()){
				...
				if($request->hasFile('logo')){
	                ...
	                $file->move(public_path().'/upload/logo',$file_name);
	                $input['logo'] = '/upload/logo/'.$file_name;
		    	}
				$information->update($input);
				return response()->json(['error'=> false, 'message' => 'Chỉnh sửa thông tin website thành công', 'reload_html'=>$information]);
			}
		} catch (Exception $e) {
            return response()->json(['error'=> true, 'message' => 'Lấy dữ liệu thất bại']);
        }
   ...
   //          } else {
				...
				$information->update();
				return response()->json(['error'=> false, 'message' => 'Chỉnh sửa cấu hình website thành công', 'reload_html'=>$information]);
			// }
		} catch (Exception $e) {
            return response()->json(['error'=> true, 'message' => 'Lấy dữ liệu thất bại']);
        }
