...
/* ------------------------------------ Danh sách các modules ------------------------------------- */
		...
		if ( $action_type == 'viewModule' ) {
			...
			try{
                ...
                if($request->hasFile('input_avatar')){
	                ...
	                $file->move(public_path().$path,$request->input('input_name_controller').'.'.$file_extension);
	                $module->avatar = $path.$request->input('input_name_controller').'.'.$file_extension;
		    	}
	...
	public function dataTable(){
		$modules = SysListModules::orderBy('active')->orderBy('id_group')->get();
		$html = '';
		if($modules){
